...
\begin{proposition}
  ...
  Assume $G$ is regular of degree $\de$ and $V(G)$ is finite.
  Then, $\|a|_{\const^\perp}\| = \de$ iff $G$ is disconnected or bipartite.
\end{proposition}

\begin{proof}
  By Lemma \ref{lem:norm_perp_is_eig},
  there exists a function $f : V(G) \rightarrow \C$ and a real number $\lambda$
  such that $af=\lambda f$, $|\lambda|=\de$ and $\sum_{x \in V(G)} f(x)=0$.
  Since $\lambda \in \R$, either $\lambda = \de$ or $\lambda=-\de$.
  \begin{itemize}
    \item If $\lambda = \de$, then since $f$ is not constant (otherwise it would be
    equal to $0$ as $\sum_{x \in V(G)}f(x)=0$), the multiplicity of
    the eigenvalue $\de$ for $a$ is at least $2$. By Proposition
    ...
    \item If $\lambda = -\de$, then $-\de$ is an eigenvalue of $a$.
...
for the permutation model introduced in Definition \ref{def:proba_on_graphs}.

\begin{theorem}
  \label{thm:Friedman}
  \uses{def:proba_on_graphs}
  \notready
  Let $r \geq 2$ be a integer and $\de = 2r$.
  For any $\epsilon > 0$,
\begin{equation}
    \lim_{N \rightarrow \infty}
    \ProbG(\|a|_{\const^\perp}\| \leq 2 \sqrt{2\de -1} + \epsilon) = 1.
\end{equation}
\end{theorem}
...
\begin{proposition}
  ...
  Let $r \geq 2$ be an integer and $\de = 2r$.
  Then,
  \begin{equation}
    \lim_{N \rightarrow \infty} \ProbG(G_{\vec{\pi}} \text{ is connected})=1
  \end{equation}
  and
    \begin{equation}
    \lim_{N \rightarrow \infty} \ProbG(G_{\vec{\pi}} \text{ is bipartite})=0.
  \end{equation}
\end{proposition}
...
\begin{proof}
  ...
  \begin{equation*}
    \ProbG(A \cup B) = \ProbG(\|a|_{\const^\perp}\| = 2r).
  \end{equation*}
  ...
  Hence
  \begin{equation*}
    \ProbG(A \cup B) \leq \ProbG(\|a|_{\const^\perp}\| > 2 \sqrt{2\de -1} + \epsilon)
  \end{equation*}
  which goes to $0$ as $N \rightarrow \infty$ by Theorem \ref{thm:Friedman}.
  So $\lim_N \ProbG(A) = 0$ and $\lim_N \ProbG(B)=0$ which allows to conclude.
\end{proof}

\section{The infinite regular tree}
\label{sec:tree}

[TODO: prove that the spectral density of the $d$-regular tree is given by the Kesten-McKay
distribution]
