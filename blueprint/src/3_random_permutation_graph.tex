...
\begin{definition}
  ...
  Let $(\Omega_{k,n},\mathcal{F}_{k,n})$ be the set of $2k$-regular graphs with $V(G) = [n]$,
  with the power-set $\sigma$-algebra.
  We define a probability measure on $\Omega_{k,n}$ by pushing forward $\Prob$
  with the function
  $\perm{n}^k \rightarrow \Omega_{k,n}$ defined in Definition \ref{def:graph_of_pi}.
\end{definition}
