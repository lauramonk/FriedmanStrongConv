...
\begin{definition}
  \label{def:rooted_tree_parent}
  \uses{def:rooted_tree}
  Let $(G,r)$ be a rooted tree.
  Let $x \in V(G)$.
  A vertex $y \in V(G)$ is a \emph{parent} of $x$ if $y$ is adjacent to $x$
  and $y$ belongs in the support of a path from $r$ to $x$.
\end{definition}

\begin{proposition}
  \label{prop:rooted_tree_unique_parent}
  \uses{def:rooted_tree_parent}
  Let $(G,r)$ be a rooted tree.
  Any vertex $x \in V(G)$, $x \neq r$,  has a unique parent.
  The root has no parent.
\end{proposition}

\begin{definition}
  \label{def:rooted_tree_children}
  \uses{def:rooted_tree_parent}
  Let $(G,r)$ be a rooted tree.
  The \emph{children} of $x \in V(G)$ are all the adjecent vertices which
  are not its parent.
\end{definition}
...
\begin{proof}
  Take $y \in C_{r+1}(x)$. Then there exists a dartwalk $(d_i)_{1 \leq i \leq r+1}$,
  from $x$ to $y$.
  We take $z=\spt(d_{r+1})$ and check it is adjacent to $y$ and lies in $C_r(x)$.
\end{proof}
