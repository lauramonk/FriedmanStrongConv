...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item the same edge links the same vertices.
  \end{itemize}
\end{definition}
...
\begin{proposition}
  ...
  \uses{lem:reachable_reflexive,lem:reachable_symmetric,
  lem:reachable_transitive}
  The reachability relation is an equivalence relation.
\end{proposition}

\begin{lemma}
  \label{lem:reachable_reflexive}
  \uses{def:reachable}
  \leanok
...
\begin{lemma}
    \label{lem:reachable_transitive}
  \uses{def:reachable}
  \leanok
...
\begin{lemma}
  ...
  \uses{def:connected_component,prop:reachable_is_equivalence}
  The connected components of $G$ are exactly the equivalence classes of
...
\begin{proof}
  Let $x, y \in V(G)$ such that $x$ and $y$ are reachable.
  By Definition \ref{def:reachable}, there exists a walk $(v_i)_{0 \leq i \leq m}$,
...
\begin{proof}
  ...
  By Proposition \ref{prop:cc_of_a_vertex}, $y$ and $x$ are reachable.
...
\begin{proof}
  Let $U$ be a connected component of $G$.
  Let $x,y \in V(G)$ be two adjacent vertices.
  \begin{itemize}
    \item If $x \in U$, then by Lemma \ref{lem:adjacent_same_cc}, $y \in U$
    and hence $\const_U(x)=1=\const_U(y)$.
    \item Otherwise, $y$ does not lie in $U$ (because $y \in U \Rightarrow x \in U$ by
    Lemma \ref{lem:adjacent_same_cc}) and hence $\const_U(x)=0=\const_U(y)$.
  \end{itemize}
\end{proof}

\begin{lemma}
  \label{lem:partition_unity_cc}
  \uses{prop:cc_partition,def:indicator_function}
  We have $$\const = \sum_{U \text{ connected component}} \const_U.$$
\end{lemma}

\begin{proof}
  Let $x \in V(G)$. By Lemma \ref{lem:partition_unity_cc}, there exists a unique
  connected component of $G$ containing $x$. Hence, $\sum_{U \text{ cc}} \const_U(x)=1$.
\end{proof}

\begin{proposition}
  \label{prop:basis_locally_constant}
  \uses{lem:locally_constant_cc,lem:partition_unity_cc}
  The family $\{\const_U, U \text{ connected component of }G\}$ is a basis
  of the space of locally constant functions.
\end{proposition}

\begin{proof}
  First,  for any connected component $U$, $\const_U$ is locally
  constant by Lemma \ref{lem:indicator_cc_loc_const}.

  Let $f$ be a locally supported function.
  We have by Lemma \ref{lem:partition_unity_cc}, for any $x \in V(G)$,
  \begin{equation*}
    f(x) = \sum_{U \text{ cc}} f(x) \const_U(x).
  \end{equation*}
  By Lemma \ref{lem:locally_constant_cc}, for every connected component $U$,
  there exists a constant $c_U$ such that, for all $x \in U$, $f(x)=c_U$
  (take $c_U$ to be $f(y_U)$ for a $y_U \in U$, which exists as $U$ is not empty).
  Then
    \begin{equation*}
    f(x) = \sum_{U \text{ cc}} c_U \const_U(x)
  \end{equation*}
  which means that $f = \sum_{U \text{ cc}} c_U \const_U$.
\end{proof}
