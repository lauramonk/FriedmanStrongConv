...
\begin{lemma}
  ...
  \begin{itemize}
    ...
    \item We have a map $e \in E(G) \mapsto d_e \in D(G)$ such that, for all $e$,
    if $e$ links $x$ and $y$, then $d_e$ is a dart from $x$ to $y$ carried by $e$.
    \item For any $d, d'$, if $\edge(d)=\edge(d')$ then $d=d'$ or $d=\symm(d')$.
    \item We have $d_1 = d_2$ iff $\spt(d_1) = \spt(d_2)$ and $\edge(d_1) = \edge(d_2)$
    (and same characterisation using the endpoints and edges).
  \end{itemize}
\end{lemma}
...
\begin{lemma}
  ...
  \uses{def:dartset_pair}
  Let $x, y \in V(G)$.
  The map
  \begin{equation*}
    \begin{split}
      \dartset(x,y) & \rightarrow E(G) \\
      d & \mapsto \edge(d)
    \end{split}
  \end{equation*}
  has range $\{e \in E(G) : e \text{ links } x \text{ and } y\}$.
  It is $2-1$ if $x=y$ and $1-1$ otherwise.
\end{lemma}
...
\begin{theorem}
  ...
  The graph $G$ constructed in Definition \ref{def:graph_of_dart} is a graph,
  and $D(G) = D$.
\end{theorem}
...
\begin{definition}
  \label{def:subgraph}
  A \emph{subgraph} $H$ of $G$ is a graph (of the same type as $G$) defined by:
  \begin{itemize}
...
\begin{definition}
  ...
  \uses{def:subgraph}
  We define the \emph{union} $H_1 \cup H_2$ of two subgraphs by taking the union of
  their respective vertex sets and edge sets,
  and the intersection $H_1 \cap H_2$ by taking their intersection.
\end{definition}
...
\begin{definition}
  \label{def:finite}
  A graph $G$ is \emph{finite} if $V(G)$ and $E(G)$ are both finite sets.
\end{definition}
...
\begin{definition}
  ...
  \uses{def:dartset}  \leanok
  \lean{Graph.LocallyFinite}
  Let $x \in V(G)$.
  We say $G$ is \emph{locally finite} at $x$ if $\dartset (x)$ is finite.
  The graph $G$ is \emph{locally finite} if it is locally finite at every vertex.
\end{definition}
...
\begin{definition}
  ...
  The \emph{edegree} of $x$ is defined as $\mathrm{edeg}(x) = \# \dartset(x)$, which is
  an element of $\mathbb{N} \cup \{ \infty\}$.
  The \emph{degree} of $x$ is the natural-number projection of the edegree.
\end{definition}
...
Several conventions can be used to define walks on a non-simple graphs, essentially
due to the presence of loops. Our convention (essential for our applications)
is that each loop can be traversed with two different orientations, which hence
correspond to different walks. For this reason, it will be convenient to define
walks using darts, as they differentiate between these two options.

\begin{definition}
  \label{def:dartwalk}
  \leanok
  \lean{Graph.Dartwalk}
  Let $x, y \in V(G)$. A \emph{dartwalk} from $x$ to $y$ on $G$ is given by a finite list
  $(d_1, \ldots, d_m)$ of darts, satisfying the following:
    \begin{itemize}
      \item $\spt(d_1)=x$ and $\ept(d_m)=y$;
      \item for any $i \in \{1, \ldots, m-1\}$, $\ept(d_{i+1}) = \spt(d_i)$.
    \end{itemize}
\end{definition}
...
\begin{definition}
  ...
  \uses{def:dartwalk}
  \leanok
  \lean{Graph.Dartwalk.length}
  The \emph{length} of a walk is its number of darts.
\end{definition}
...
\begin{definition}
  ...
  \uses{def:dartwalk}
  Let $w = (d_i)_{1 \leq i \leq m}$ be a dartwalk from $x$ to $y$
  and $w' = (d_i')_{1 \leq i \leq m'}$ be a dartwalk from $y$ to $z$.
  We define the \emph{concatenation} $w \cdot w'$ to be the dartwalk $(d_i'')_{1 \leq i \leq m+m'}$
  where $d_i''=d_i$ for $i \leq m$ and $d_{i-m}'$ for $i > m$.
\end{definition}

\begin{lemma}
  \label{lem:length_concat}
  \uses{def:concatenation,def:length}
...
\begin{definition}
  ...
  We define a binary relation on $V(G)$ by: $x \sim y$ if there exists a dartwalk
...
\begin{lemma}
  ...
  \uses{def:reachable,lem:adjacent_vs_dart}
  \leanok
...
\begin{definition}
  \label{def:dartpath}
  \uses{def:dartwalk}
  A dartwalk $(d_i)_{1 \leq i \leq m}$ is called a \emph{path} if
  for all $1 \leq i,j \leq m$, $\spt(d_i) = \spt(d_j) \Rightarrow i=j$.
\end{definition}

\begin{definition}
  \label{def:tree}
  The graph $G$ is a \emph{tree} if, for all $x$, $y \in V(G)$,
  there exists a unique path from $x$ to $y$.
\end{definition}
...
\begin{lemma}
  \label{lem:bound_cc}
  \uses{prop:cc_is_partition,def:finite}
  If $V(G)$ is finite, then $\cc(G)$ is finite and $\#V(G) \leq \# \cc(G)$.
\end{lemma}

\subsection{Morphisms between graphs}

[Question: the following notion of morphism does not contain morphisms which
reverse loops. Should they be included?]

Let $G_1$ and $G_2$ be two graphs.


\begin{definition}
  \label{def:function_graphs}
  A \emph{function} $f : G_1 \rightarrow G_2$ is a pair of functions $f_V : V(G_1) \rightarrow V(G_2)$
  and $f_E : E(G_1) \rightarrow E(G_2)$.
  We will denote $f_V(x)=f(x)$ for $x \in V(G_1)$ and $f_E(e)=f(e)$ for $e \in E(G_1)$,
  i.e. view $f$ as a function from $V(G_1) \cup E(G_1)$ to $V(G_2) \cup E(G_2)$
  sending vertices on vertices and edges on edges.
\end{definition}

In the following, the usual operations and definitions on functions are
with respect to the function $f : V(G_1) \cup E(G_1) \rightarrow V(G_2) \cup E(G_2)$.

\begin{definition}
  \label{def:graph_morphism}
  \uses{def:function_graphs}
  A \emph{morphism} $f : G_1 \rightarrow G_2$ is
  a function $f : G_1 \rightarrow G_2$ such that for all
  $x, y \in V(G_1)$, $e \in E(G_1)$,
  if $e$ links $x$ and $y$ in $G_1$, then $f(e)$ links $f(x)$ and $f(y)$ in $G_2$.
\end{definition}

\begin{definition}
  \label{def:graph_isomorphism}
  \uses{def:graph_morphism}
  A morphism $f : G_1 \rightarrow G_2$ is an \emph{isomorphism} if
  there exist a morphism $g : G_2 \rightarrow G_1$ which is the inverse of $f$.
  In this case we denote $f^{-1}$ this inverse.
\end{definition}

\begin{definition}
  \label{def:local_isomorphism}
  \uses{def:graph_morphism}
  A morphism $f : G_1 \rightarrow G_2$ is a \emph{local isomorphism}
  if for all $x \in V(G_1)$, the restriction of $f$ to $\incset(x)$ is
  a bijection onto $\incset(f(x))$.
\end{definition}

\begin{definition}
  \label{def:covering_map}
  \uses{def:local_isomorphism}
  A morphism $f : G_1 \rightarrow G_2$ is a \emph{covering map} if it is
  surjective and a local isomorphism.
\end{definition}

[todo: dart version]

\begin{definition}
  \label{def:covering_graph}
  \uses{def:covering_map}
  A graph $G_2$ is a \emph{covering graph} of a graph $G_1$ if there exists
  a covering map $\pi : G_2 \rightarrow G_1$.
\end{definition}

\subsection{Rooted graphs}

\begin{definition}
  \label{def:rooted_graph}
  A \emph{rooted graph} is a graph $G$ with a distinguished vertex $r$, its \emph{root}.
\end{definition}

\begin{definition}
  \label{def:rooted_graph_parent}
  \uses{def:rooted_graph}
  Let $(G,r)$ be a rooted graph.
  Let $x \in V(G)$.
  A vertex $y \in V(G)$ is a \emph{parent} of $x$ if $y$ is adjacent to $x$ and $\dist(r,y) = \dist(r,x)-1$.
\end{definition}


\begin{definition}
  \label{def:rooted_graph_child}
  \uses{def:rooted_graph_parent}
  Let $(G,r)$ be a rooted graph.
  A \emph{child} of $x \in V(G)$ is an adjacent vertice which is not a parent.
\end{definition}

\begin{lemma}
  \label{lem:root_no_parent}
  \uses{def:rooted_graph_parent}
  The root of a rooted graph has no parent.
\end{lemma}

\begin{lemma}
  \label{lem:exists_parent}
  \uses{def:rooted_graph_parent}
  Let $(G,r)$ be a rooted graph. Assume $G$ is connected.
  Any vertex $x \in V(G)$ distinct from the root admits a parent.
\end{lemma}

\begin{definition}
  \label{def:rooted_tree}
  \uses{def:rooted_graph}
  A \emph{rooted tree} is a rooted graph $(G,r)$ which is a tree.
\end{definition}

\begin{proposition}
  \label{prop:rooted_tree_unique_parent}
  \uses{lem:exists_parent,def:rooted_tree}
  Let $(G,r)$ be a rooted tree.
  Any vertex $x \in V(G)$ distinct from the root admits a unique parent.
\end{proposition}


\subsection{Nonbacktracking walks and reduction}

\begin{definition}
  \label{def:nonbacktracking}
  \uses{def:dartwalk}
  A dartwalk $(d_i)_{1 \leq i \leq m}$ is called \emph{non-backtracking} if
  for all $1 \leq i < m$, $d_{i+1} \neq \symm (d_{i})$.
\end{definition}

\begin{definition}
  \label{def:walk_reduction}
  \uses{def:dartwalk}
  Let $w = (d_i)_{1 \leq i \leq m}$ be a dartwalk.
  We say $w$ \emph{admits a reduction} if there exists $1 \leq j < m$ such that
  $d_{j+1} = \symm(d_j)$.
  In this case, we call the dartwalk $w'=(d_i')_{1 \leq i \leq m-2}$ obtained by
  letting $d_i'=d_i$ for $i<j$ and $d_i'=d_{i+2}$ for $i \geq j$ a \emph{reduction} of $w$.
\end{definition}
...
\begin{definition}
  ...
  \uses{def:dartwalk}
  Let $x, y \in V(G)$.
  Two dartwalks $w = (d_i)_{1 \leq i \leq m}$ and $w' = (d_i')_{1 \leq i \leq m'}$ from $x$ to $y$
  are said to be \emph{homotopic} if
  there exists a finite sequence $(w_j)_{1 \leq j \leq n}$ such that:
  \begin{itemize}
    \item for all $1 \leq j \leq n$, $w_j$ is a
    dartwalk from $x$ to $y$;
    \item $w_1=w$ and $w_n=w'$;
    \item for any $1 \leq j < n$, either $w_j$ is a reduction of $w_{j+1}$ or $w_{j+1}$ is a
    reduction of $w_j$.
  \end{itemize}
\end{definition}

\begin{proposition}
  \label{prop:homotopy_equivalence}
  The homotopy relation is an equivalence relation.
\end{proposition}

\begin{theorem}
  \label{thm:unique_nonbacktracking}
  \uses{def:nonbacktracking,prop:homotopy_equivalence}
  Let $x, y \in V(G)$.
  Any dartwalk from $x$ to $y$ is homotopic to a unique nonbacktracking dartwalk, which we denote by
  $w_{\mathrm{nb}}$.
\end{theorem}
...
\begin{definition}
  \label{def:universal_cover}
  \uses{def:graph_of_dart,def:concatenation,thm:unique_nonbacktracking}
  We define the \emph{universal cover} $\hat{G}$ of $(G,r)$, which is a rooted graph,
  by letting:
  \begin{itemize}
    \item $V(\hat G)$ be the set of non-backtracking dartwalks on $G$
    starting at $r$;
    \item the dartset $D(\hat{G})$ is defined as the set of pairs $(w,d)$ where
    $w$ is a non-backtracking dartwalk ($w \in V(\hat{G})$) and $d \in D(G)$
    is a dart on $G$ starting at the endpoint of $w$;
    \item for $(w,d) \in D(\hat{G})$, $\spt_{\hat{G}}(w,d) = w$,
    $\ept_{\hat{G}}(d) = (w \cdot d)_{\mathrm{nb}}$ and $\symm_{\hat{G}}(w,d) =
     ((w \cdot d)_{\mathrm{nb}},\symm_G(d))$;
    \item the root $\hat{r}$ is the empty walk from $r$ to $r$.
  \end{itemize}
\end{definition}

\begin{theorem}
  \label{thm:univ_cover_is_tree}
  \uses{def:universal_cover,def:tree}
  The universal cover $(\hat{G}, \hat{r})$ of $(G,r)$ is a rooted tree.
\end{theorem}

\begin{theorem}
  \label{thm:univ_cover_cover}
  \uses{def:universal_cover,def:covering_map}
  The projection map $\pi : \hat{G} \rightarrow G$ which is defined by $\pi(w)=\ept(w)$
  for $w \in V(\hat G)$ and $\pi(w,d)=d$ for $(w,d) \in D(\hat G)$ is a covering map.
\end{theorem}
...
\begin{definition}
  ...
  between $x$ and $y$
  is the length of the shortest dartwalk from $x$ to $y$ (with value in
  $\N \cup \{\infty\}$).
\end{definition}
...
\begin{lemma}
  ...
  \uses{def:distance}
  Let $x, y \in V(G)$, we assume $\dist(x,y) \neq 0$. Then there exists a dartwalk
...
\begin{lemma}
  ...
  \uses{def:connected,def:diameter,def:finite}
...
\begin{proof}
  ...
  Since $V(G)$ is finite all functions $V(G) \rightarrow \C$ are in $\ell^2(G)$.
