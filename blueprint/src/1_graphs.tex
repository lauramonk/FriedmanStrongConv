...
\begin{definition}
  ...
  $G$ is said to be \emph{locally finite} if it is locally finite at every vertex.
\end{definition}
...
Let $G$ be a graph.
We will be interested in the Hilbert space $\ell^2(V(G))$, which is defined as the set of
...
The space $\B(\ell^2(V(G)))$ of bounded linear operators on $\ell^2(V(G))$ is a unital $\C^*$
algebra.
