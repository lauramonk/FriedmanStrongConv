...
\begin{definition}
  ...
  We say $G$ is \emph{regular of degree $d$} if it is locally finite and,
...
\begin{lemma}
  \label{lem:constant_function_eq_zero}
...
\begin{proof}
  ...
  \begin{align*}
    a \const(x)
    & = \sum_{e \in \incset(x)} \const(\incother(e, x)) + \# \loopset(x) \times \const(x)
    & \text{by Definition \ref{def:adjacency_operator}} \\
    & = \sum_{e \in \incset(x)} 1 + \# \loopset(x) \times 1
...
\begin{proposition}
  \label{prop:adjacency_bounded_iff}
  The adjacency matrix of $G$ is a bounded operator on $\ell^2(G)$ if and only
  if $G$ is locally bounded.
\end{proposition}

We will also prove the following upper bound on the operator norm of $a$.

\begin{proposition}
  \label{prop:norm_adjacency_sup_degree}
  Assume $G$ is locally bounded. Then $\|a\| \leq \sup_{x \in V(G)} \deg(x)$.
\end{proposition}
...
\begin{lemma}
  ...
  \begin{equation}
    |af(x)|^2
    \leq \deg(x) \left( \sum_{e \in \incset(x)} |f(\incother(e,x))|^2
    + \# \loopset(x) |f(x)|^2 \right).
  \end{equation}
\end{lemma}

\begin{proof}
  Let $x \in V(G)$. By Definition \ref{def:adjacency_operator},
  \begin{equation}
    |af(x)|^2
    = \left| \sum_{e \in \incset(x)} f(\incother(e, x)) + \sum_{e \in \loopset(x)}
    f(x) \right|^2.
  \end{equation}
  By \mathlibok{Cauchy-Schwarz},
  \begin{multline*}
    |af(x)|^2
    \leq
    \left(\sum_{e \in \incset(x)} 1 + \sum_{e \in \loopset (x)} 1
    \right)\\
      \times
    \left( \sum_{e \in \incset(x)} |f(\incother(e, x))|^2
    + \sum_{e \in \loopset (x)} |f(x)|^2
    \right).
  \end{multline*}
...
\begin{lemma}
  ...
  \uses{lem:constant_is_l2,lem:adjacency_of_constant_regular,lem:constant_function_eq_zero}
  Assume $G$ is finite and not empty.
  If $G$ is regular of degree $d$, then $d$ is an eigenvalue of the adjacency
...
\begin{proof}
  ...
  then $a \const = d \const$, which is enough to conclude by the definition
  of eigenvalue and eigenvector, since $\const \neq 0$ by Lemma \ref{lem:constant_function_eq_zero}.
\end{proof}

\begin{proposition}
  \label{prop:norm_adjacency_regular}
  \uses{prop:norm_adjacency_sup_degree,lem:constant_eigenfunction}
  We assume that $G$ is finite, not empty and $d$-regular. Then $\|a\|=d$.
\end{proposition}

\begin{proof}
  Since $G$ is $d$-regular, for all $x \in V(G)$, $\deg(x) \leq d$. Then, by
