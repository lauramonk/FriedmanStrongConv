...
\begin{proof}
  This is simply Lemma \ref{lem:bound_a_sup_degree}.
\end{proof}
...
Let $G$ be a regular graph of degree $d$, which we assume to be finite.
...
\begin{theorem}
  \uses{lem:spectrum_regular_in_segment,lem:constant_eigenfunction_regular}
  The spectrum of the adjacency operator is a family of $n$ eigenvalues
  $$d = \lambda_1 \geq \lambda_2 \geq \ldots \geq \lambda_n \geq -d.$$
\end{theorem}

\begin{proof}
  This is the spectral theorem in finite dimension together with
  Lemma \ref{lem:spectrum_regular_in_segment} and \ref{lem:constant_eigenfunction_regular}.
\end{proof}

Now we can be more precise about special cases.

\begin{proposition}
  \label{prop:regular_multiplicity_trivial_eigenvalue}
  \uses{def:number_connected_components}
  The multiplicity of $d$ as an eigenvalue is the number of connected components of $G$.
\end{proposition}
...
\begin{proposition}
  ...
  \uses{def:regular,def:bipartite}
  The number $-d$ is an eigenvalue of the adjacency operator iff the graph is bipartite.
\end{proposition}
