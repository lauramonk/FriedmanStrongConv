...
\begin{lemma}
  ...
  \begin{itemize}
    ...
    \item For any $e \in E(G)$, there exists $d \in D(G)$ such that
          $\edge^{-1}(\{e\}) = \{d, \symm(d)\}$.
...
\begin{lemma}
  ...
  \uses{lem:dart_API,def:dartset_pair}
  Let $x,y \in V(G)$.
  The range of the restriction of the function $\edge : D(G) \to E(G)$
  to $\dartset(x,y)$
...
\begin{proof}
  ...
  Now take $e \in E(G)$ linking $x$ and $y$. By Lemma \ref{lem:dart_API},
...
\begin{definition}
  ...
  We say $G$ is \emph{locally finite} if, for all $x$, $\dartset (x)$ is finite.
\end{definition}
...
\begin{definition}
  ...
  A dartwalk $(d_i)_{1 \leq i \leq m}$ is called a \emph{dartpath} if all vertices in its
