...
\subsection{Paths and distance}

We define a notion of walk on the graph, and a corresponding distance function.
Let $G$ be a graph.
...
\begin{definition}
  \label{def:connected}
    \uses{def:walk}
  We say $G$ is \emph{connected} if, for all $x$, $y \in V(G)$, there exists a walk from $x$ to $y$.
\end{definition}
