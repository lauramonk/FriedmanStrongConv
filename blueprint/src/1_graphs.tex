...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item an ordered sequence of $m+1$ vertices $v_0, \ldots, v_m \in V(G)$;
    \item an ordered sequence of $m$ edges $e_1, \ldots, e_m \in E(G)$;
  \end{itemize}
...
\begin{definition}
  \label{def:connected_vertices}
  \uses{def:walk}
  We say two vertices are connected if there exists a walk from one to the other in $G$.
\end{definition}

\begin{proposition}
  \label{prop:vertex_connectedness_eq_rel}
  \uses{def:connected_vertices}
  The vertex connectedness relation is an equivalence relation.
\end{proposition}
