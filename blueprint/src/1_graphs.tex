...
\begin{lemma}
  ...
  \end{equation}
  Then the adjacency operator $a$ is bounded on $\ell^2(G)$ and $\| a \| \leq M$.
\end{lemma}
...
\begin{proof}
  ...
  Lemma \ref{lem:adjacency_bounded}, $a$ is bounded and $\|a \| \leq d$.
  By Lemma \ref{lem:constant_is_l2}, since $G$ is finite, $\const$ is an element of $\ell^2(G)$.
  By Lemma \ref{lem:adjacency_of_constant}, $a \const = d \const$, and hence $\|a \| \geq d$ by definition
